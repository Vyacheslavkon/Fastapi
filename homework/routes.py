...
import os
from homework.database import get_db, engine, async_session
...
@asynccontextmanager
async def lifespan(application: FastAPI, eng=engine, session: AsyncSession = Depends(get_db)):
...
app = FastAPI(lifespan=lambda app_instance: lifespan(
    app_instance, engine
))
...
async def add_recipe(recipe: schemas.AddRecipe,session: AsyncSession = Depends(get_db)) -> Recipe:
...
async def recipes(session: AsyncSession = Depends(get_db)) -> Sequence[Recipe]:
    res = await session.execute(select(Recipe).order_by(Recipe.views.desc(),
                                                        Recipe.cook_time))
